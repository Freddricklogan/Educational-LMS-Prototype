...
export function makeCharts(Chart) {
  ...
  return {
    ...
    scatterLabelled(canvas, points, xTitle, yTitle) {
      const focus = points.filter((p) => p.focus);
      const rest = points.filter((p) => !p.focus);
      mount(canvas, {
        type: 'scatter',
        data: { datasets: [
          { label: 'Peers', data: rest.map((p) => ({ x: p.x, y: p.y, label: p.label })), backgroundColor: tokens().muted, pointRadius: 4 },
          { label: 'Selected', data: focus.map((p) => ({ x: p.x, y: p.y, label: p.label })), backgroundColor: tokens().accent, pointRadius: 8, pointStyle: 'rectRot' }
        ] },
        options: { ...base(xTitle, yTitle, true), plugins: { legend: { labels: { color: TICK } }, tooltip: { callbacks: { label: (c) => `${c.raw.label}: ${c.raw.x.toFixed(1)}%, ${c.raw.y.toFixed(1)}%` } } } }
      });
    },
