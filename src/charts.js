  ...
  return {
    ...
    scatterLabelled(canvas, points, xTitle, yTitle) {
      ...
      mount(canvas, {
        type: 'scatter',
        data: { datasets: [
          { label: 'Peers', data: rest.map((p) => ({ x: p.x, y: p.y, label: p.label })), backgroundColor: '#8b98b0', pointRadius: 4 },
          { label: 'Selected', data: focus.map((p) => ({ x: p.x, y: p.y, label: p.label })), backgroundColor: '#58A6FF', pointRadius: 8, pointStyle: 'rectRot' }
        ] },
